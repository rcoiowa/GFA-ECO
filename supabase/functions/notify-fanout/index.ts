...
Deno.serve(async (req) => {
  ...
  if (!verified) {
    return new Response(JSON.stringify({ ok: true, external: 'skipped_unverified' }), {
      status: 200,
    });
  }
  ...
  async function claimChannel(channel: 'email' | 'sms'): Promise<boolean> {
    const { error } = await admin
      .from('v2_notification_deliveries')
  ...
  try {
    ...
    if (!user)
      return new Response(JSON.stringify({ ok: true, external: 'no_user' }), { status: 200 });
      ...
      if (await claimChannel('email')) {
        jobs.push(
          fetch('https://api.resend.com/emails', {
            ...
            .then((r) => {
              if (!r.ok) return markFailed('email', `resend ${r.status}`);
            })
        ...
        jobs.push(
          fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilioSid}/Messages.json`, {
            ...
            body: new URLSearchParams({
              From: twilioFrom,
              To: profile.phone,
              Body: `VRCC: ${title} — ${bodyText}`,
            }),
          })
            .then((r) => {
              if (!r.ok) return markFailed('sms', `twilio ${r.status}`);
            })
