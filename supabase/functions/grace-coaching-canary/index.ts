...
const ALLOWED = new Set(
  (Deno.env.get('CANARY_ALLOWED_SUBS') ?? '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean),
);
...
const ACTIONS: Record<string, (sb: Client, p: Record<string, unknown>) => Promise<unknown>> = {
  async assignParticipantCoach(sb, p) {
    return rpc(sb, 'assign_participant_coach', {
      p_participant_person_id: p.participantPersonId,
      p_coach_person_id: p.coachPersonId,
  ...
  async createSupportRequest(sb, p) {
    ...
    const { data, error } = await sb
      ...
      .insert({
        ...
        focus: p.focus ?? null,
        preferred_modality: p.preferredModality ?? 'video',
        status: 'open',
        organization_id: 1,
        program_id: 1,
      })
    ...
    return error
      ...
      : { ok: true, code: 'created', ...data };
  },
  async createBookingRequest(sb, p) {
    return rpc(sb, 'create_booking_request', {
      p_participant_person_id: p.participantPersonId,
      p_provider_person_id: p.providerPersonId,
      p_support_request_id: p.supportRequestId ?? null,
      p_service_type_id: p.serviceTypeId ?? 1,
      p_modality: p.modality ?? 'video',
      p_duration_minutes: p.durationMinutes ?? 50,
      p_note: p.note ?? null,
      p_starts: p.proposedStarts,
    });
  },
  async proposeBookingTimes(sb, p) {
    return rpc(sb, 'propose_booking_times', {
      p_booking_request_id: p.bookingRequestId,
      p_starts: p.starts,
    });
  },
  async counterProposeBookingTimes(sb, p) {
    return rpc(sb, 'counter_propose_booking_times', {
      p_booking_request_id: p.bookingRequestId,
      p_starts: p.starts,
    });
  },
