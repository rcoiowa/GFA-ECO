...
-- --------------------------------------------------------------------------
-- Identity crosswalk: v2 auth-uuid  <->  canonical person_id
-- v2_profiles.id == auth.users.id == people.auth_user_id, so the exact match is
-- deterministic. Ambiguous/unmapped rows are recorded, never guessed.
-- --------------------------------------------------------------------------
create table recoveryos.v2_identity_map (
  id            bigint generated always as identity primary key,
  v2_profile_id uuid not null unique,
  person_id     bigint references recoveryos.people(id) on delete set null,
  confidence    text not null check (confidence in ('exact','high','ambiguous','unmapped')),
  mapped_via    text not null,
  created_at    timestamptz not null default now()
);
create index v2_identity_map_person_idx on recoveryos.v2_identity_map(person_id);
alter table recoveryos.v2_identity_map enable row level security;
