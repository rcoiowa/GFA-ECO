...
| Onboarding — intake (consent ack, support interest, immediate need)               | NOT IMPLEMENTED              | `OnboardingPage` collects 0/7; pathway/consent/faith are optional post-login pages (`ProfilePage`, `PrivacyConsentPage`)                                                                                                     | no gated intake step                                                                                                          | add a minimal onboarding step (consent acknowledgment + "what brings you here" + immediate-support prompt) | **YES (privacy/dignity)**                     |
...
| Support Now — reachable without an account                                        | PARTIALLY READY              | full ladder only in auth shells; public pages have 988/911 only (`LandingPage.tsx:58-63`)                                                                                                                                    | GFA warmline/office gated                                                                                                     | surface GFA warmline + office on public pages (safe repo add)                                              | **YES (safety)**                              |
...
| Staff grievance follow-through                                                    | PARTIALLY READY (copy fixed) | grievance row created (`residenceOperations.ts:88-104`) but **no staff grievance queue**; copy softened to the reliable paper/verbal path (`GrievancePage.tsx`)                                                              | no in-app staff grievance queue                                                                                               | add `listGrievances`-by-residence read + a Staff queue page (needs RLS check)                              | fast-follow before serving residents          |
...
1. **Support Now full ladder is account-gated (participant safety).** Anonymous visitors in crisis
   get 988/911 but not the GFA warmline/office/grounding. _Fix:_ surface the GFA warmline + office
   (deterministic `tel:` links) on public pages — safe repo add. **Not yet done** (recommended next).
2. **Password-reset completion was broken (service interruption).** **FIXED** (`b1eb4ef`).
3. **No onboarding intake / consent capture at entry (privacy/dignity).** Consent is enforced
   server-side for Grace, but a participant is never asked to acknowledge consent or state an
   immediate-support need at onboarding. _Fix:_ a minimal onboarding step. Fast-follow (repo-safe).
4. **Grievance dead-end (staff follow-through).** Copy **FIXED** to the reliable path (`b1eb4ef`);
   the durable fix (staff grievance queue) is a fast-follow before serving residents.
...
- Support Now deterministic (met) **+ GFA warmline surfaced on public pages** (fast-follow).
...
- EJWRH self-apply form; public-intake review UI; full staff grievance queue (recommended fast-follow).
...
2. **(Type B — next repo fast-follows)** (a) Surface GFA warmline + office on public pages
   (deterministic `tel:`); (b) add a minimal onboarding step (consent acknowledgment + "what brings
   you here" + immediate-support prompt) — reuse existing consent write + `ensureMyPerson`.
   Typecheck/test/build; commit.
...
| Onboarding works                              | PARTIALLY (person-creation READY; intake step recommended)    |
| Consent works                                 | READY (server-enforced)                                       |
| Support Now works                             | READY (public warmline surfacing recommended)                 |
...
- RLS abuse tests (fixtures only, cleaned up). The two repo fast-follows in Execution Plan step 2
  (public warmline + minimal onboarding/consent step) can proceed immediately under existing Type B
  authority.
