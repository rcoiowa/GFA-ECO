...
6. **Counsel has answered the re-acknowledgment questions (DOC-INTEGRITY-001).** The four docs are `requires_acknowledgment=true`, and the live `ensure_my_document_assignments()` (0139) assigns on `requires_signature OR requires_acknowledgment`. So publishing the full editions (APPLY 1) is **not acknowledgment-neutral**: the next Documents-area visit by an active resident (or approved applicant) will auto-create a new assignment for the new edition and surface a re-acknowledgment. **Do not apply 0152 until counsel has cleared re-acknowledgment** — applying it effectively initiates re-acknowledgment through the ordinary resident path.

**STOP conditions:** target is not CQCX; tail lacks 0150/0151; verifier fails; any live placeholder hash differs; DA has not cleared the intake withdrawal; **counsel has not cleared re-acknowledgment** (publishing surfaces it via the ordinary resident path). Stop and report — do not improvise.
